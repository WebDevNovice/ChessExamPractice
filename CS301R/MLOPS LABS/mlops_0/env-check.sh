...
if ! curl -sf http://localhost:4000 >/dev/null 2>&1; then
  echo "❌ MLflow not reachable at http://localhost:4000"
  echo "   Common causes: port 4000 already in use, container failed to start"
  echo "   Debug: docker compose ps && docker compose logs mlflow"
  exit 1
fi

echo "✅ MLflow reachable at http://localhost:4000"
docker compose down
